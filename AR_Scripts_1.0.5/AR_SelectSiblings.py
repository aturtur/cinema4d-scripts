...
Name-US: AR_SelectSiblings
Version: 1.0
Description-US: DEFAULT: Selects the object's siblings. CTRL: Deselect original selection.
...
def GetSiblings(op):
    """
    Retrieves the siblings from the given operator.
    ...
    if op == None: # If object is none
        ...
        op = op.GetChildren() # Get children
        return op # Return the object(s)
...
def main():
    ...
    if keyMod == "None":
        for s in selection: # Loop through selection
            Select(GetSiblings(s)) # Select parent object
    elif keyMod == "Ctrl":
        for s in selection:
            Select(GetSiblings(s))
        for s in selection:
