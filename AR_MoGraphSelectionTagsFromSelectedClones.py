...
def MgSelTagsFromSelectedClones(obj):
    ...
    for k in reversed(tags): # Loop through reversed list of tags
        if k.GetName().split("_")[0] == prefix:
            x = x + 1 # Increase iteration variable
    for i in reversed(xrange(0,cnt)): # Loop through reversed list of clones
        if selection.IsSelected(i) == True:
            ...
            obj.InsertTag(t) # Insert new tag to object
