...
Name-US: AR_PrintType
Version: 1.0
Description-US: Prints info about selected objects, tags, materials, Xpresso nodes and Redshift nodes.
...
def main():
    ...
    for s in selection: # Iterate through selection
        ...
        print ("'"+s.GetName()+"'", "\""+type(s).__name__+"\"", "'"+s.GetTypeName()+"'", s.GetType()) # Print: name, class, type name and type id
    ...
    for m in materials: # Iterate through materials
        if m.GetBit(c4d.BIT_ACTIVE): # If material is selected
            print ("'"+m.GetName()+"'", "\""+type(m).__name__+"\"", "'"+m.GetTypeName()+"'", m.GetType()) # Print: name, class, type name and type id
