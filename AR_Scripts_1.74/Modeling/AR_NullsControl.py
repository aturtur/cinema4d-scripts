...
Name-US: AR_NullsControl
Version: 1.0.1
Description-US: Creates null(s) from selected point(s) that control original geometry.
...
def CreateControlNulls(obj):
    ...
    for i in range(pointCount): # Loop through points
        if(points.IsSelected(i)): # If point is selected
            ...
            # Python code
            spaces = "    "
            pythonNode[c4d.GV_PYTHON_CODE] = "import c4d\ndef main():\n"+spaces+"global Vector\n"+spaces+"Vector = Link.GetMg().off"
