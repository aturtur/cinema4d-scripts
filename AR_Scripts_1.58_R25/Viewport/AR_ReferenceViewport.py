...
Version: 1.0.1
Description-US: Creates a view port for animation reference. Shift: Create only background. Ctrl: Delete setup.
...
def main():
    ...
    if keyMod == "Ctrl": # Reference background
        ...
        for m in materials:
            if m.GetName().find("AR_REF_") != -1:
                doc.AddUndo(c4d.UNDOTYPE_DELETE, m)
                m.Remove()
        for o in objects:
            if o.GetName().find("AR_REF_") != -1:
