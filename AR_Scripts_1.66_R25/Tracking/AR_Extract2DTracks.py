...
Name-US: AR_Extract2DTracks
Version: 1.0
Description-US: Extracts 2D tracks from selected motion tracker to null objects. Default: Manual tracks Shift: Auto tracks
...
def Create2DTracks(mt, keyMod):
    ...
    if keyMod == "Shift":
        tracksStatus = tracks.GetTrackIndices(userTracks = False, autoTracks = True)
        tracksStatusString = "tracksStatus = tracks.GetTrackIndices(userTracks = False, autoTracks = True)\n"
    else:
        tracksStatus = tracks.GetTrackIndices(userTracks = True, autoTracks = False)
        tracksStatusString = "tracksStatus = tracks.GetTrackIndices(userTracks = True, autoTracks = False)\n"
...
def main():\n\
    ...
    ft = mt.GetFootageData()\n    "+tracksStatusString+"    tracksType = tracksStatus.GetAll(count)\n\
    ...
    groupNull.SetName("Extracted 2D Tracks") # Set group null's name
    ...
    for i, ut in enumerate(reversed(userTracks)): # Iterate through user trackers
        ...
        CreateUserDataInt(pyTag, "Index", len(userTracks)-1-i) # Create user data for tracker index
...
def main():
    ...
    for s in selection: # Iterate through selection
        if s.GetType() == 1028393: # If Motion Tracker object
            Create2DTracks(s, keyMod) # Create user 2D tracks
