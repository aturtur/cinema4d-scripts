...
Name-US: AR_SelectNext
Version: 1.0
Description-US: DEFAULT: Selects the next object(s). SHIFT: Keeps the old selection. CTRL: Safe mode.
...
def main():
    ...
    if keyMod == "None":
        for s in selection: # Loop through selection
            ...
            Select(GetNext(s, False)) # Select next object
    elif keyMod == "Shift":
        for s in selection:
            Select(GetNext(s, False))
    elif keyMod == "Ctrl":
        for s in selection:
            Deselect(s) # Deselect original object
            Select(GetNext(s, True))
